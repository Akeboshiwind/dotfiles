function ct --wraps=claude --description 'Run Claude Code in a Docker sandbox'
    ...
    # submodule's two differ in location but still agree. Mount the common dir,
    # or the sandbox cannot resolve the pointer in the worktree's .git file and
    # the agent gets no git at all.
    ...
    # One kit for every repository, so every sandbox is the same. Something a
    # single project needs goes into osm-base alongside everything else.
    set -l template osm-base
    set -l kits $kit_dir/$template

    # The repository path, not ~/.config/sbx/kits: the same path syn builds the
    # template from, and `find` below would report a symlink rather than walk it.
    set -l kit_args --kit $kits
